...
## 2. Activity Diagram (Node Logic)

```mermaid
flowchart TD
    Start([Start]) --> Init[Init Nav2 & Action Server]
    Init --> Wait{Wait for Goal}
    Wait -->|Goal Received| Plan[Send Waypoints to Nav2]
    Plan --> Loop{While Moving}
    
    Loop --> CheckCancel{Cancel?}
    CheckCancel -->|Yes| Abort([Abort])
    
    CheckCancel -->|No| Feedback[Get Distance Remaining]
    Feedback --> CalcPerc[Calculate % Complete]
    CalcPerc --> PubFeed[Publish Feedback]
    
    PubFeed --> HeadCtrl[Calculate Turn Rate]
    HeadCtrl --> PReg[P-Regulator: HeadYaw = K * Rate]
    PReg --> PubTrack[Publish Head Trajectory]
    
    PubTrack --> Loop
    Loop -->|Done| End([Goal Succeeded])
```

---

## 3. Head Control Regulator
To ensure smooth head movement compliant with the base rotation, we used a Proportional (P) regulator.
*   **Input:** Yaw difference ($d\psi$) between control loops (approx 0.1s).
*   **Formula:** $\theta_{head} = K_p \cdot d\psi$
*   **Parameters:** $K_p = 2.0$, clamped to $[-1.0, 1.0]$ rad.
*   **Smoothness:** Trajectory points are sent with `time_from_start = 1.0s` to ensure slow, fluid interpolation.

---

## 4. Source Code (`lab2_p2_node.py`)
