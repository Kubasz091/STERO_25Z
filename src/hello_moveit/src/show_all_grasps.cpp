...
class ShowAllGraspsNode : public rclcpp::Node {
public:
  explicit ShowAllGraspsNode(const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
    : rclcpp::Node("show_all_grasps", options),
  ...
  void run() {
    ...
    for (const auto& face : faces) {
        std::string face_name = face.first;
        ...
        for (int i_z = 0; i_z < 4; ++i_z) {
            ...
            for (int i_y = 0; i_y < 3; ++i_y) {
                ...
                T_local.linear() = (Eigen::AngleAxisd(roll, Eigen::Vector3d::UnitZ()) * Eigen::AngleAxisd(pitch, Eigen::Vector3d::UnitY())).toRotationMatrix();

                Eigen::Isometry3d T_O_Ogr = T_face * T_local;
                Eigen::Isometry3d T_BE = T_BO * T_O_Ogr * T_OF_base * T_FE;

                Eigen::Isometry3d T_E_pre_local = Eigen::Isometry3d::Identity();
                T_E_pre_local.translation() = Eigen::Vector3d(-approach_distance_, 0.0, 0.0);
                Eigen::Isometry3d T_BE_pre = T_BE * T_E_pre_local;

                {
                    ...
                    shared_data_.face_name = face_name;
                ...
                RCLCPP_INFO(get_logger(), "Face: %s | Grasp %d: Roll=%.0f, Pitch=%.0f", face_name.c_str(), grasp_idx + 1, roll * 180.0/M_PI, pitch * 180.0/M_PI);
  ...
  void visualization_loop(const moveit::core::JointModelGroup* jmg) {
      ...
      while (rclcpp::ok() && viz_running_) {
          ...
          if (local_data.initialized) {
              ...
              mvt_->publishAxisLabeled(local_data.T_BE,
                  "G" + std::to_string(local_data.id) + "_" + local_data.face_name,
...
int main(int argc, char * argv[]) {
  ...
  auto node = std::make_shared<ShowAllGraspsNode>(options);
