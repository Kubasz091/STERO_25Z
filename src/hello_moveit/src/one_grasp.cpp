...
class OneGraspNode : public rclcpp::Node {
public:
  explicit OneGraspNode(const rclcpp::NodeOptions & options = rclcpp::NodeOptions()) : rclcpp::Node("one_grasp", options),
  ...
  void run() {
    ...
    if (auto p = call_get_pose(table_entity_)) {
